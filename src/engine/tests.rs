...
#[test]
fn report_generation_timeout_scales_with_the_closed_payload() {
    let limits = EngineLimits::default();
    assert_eq!(
        limits.report_attempt_timeout_for_payload(REPORT_PAYLOAD_BASE_BYTES),
        REPORT_BASE_ATTEMPT_TIMEOUT_MS
    );
    assert_eq!(
        limits.report_attempt_timeout_for_payload(REPORT_PAYLOAD_BASE_BYTES + 1),
        REPORT_BASE_ATTEMPT_TIMEOUT_MS + REPORT_PAYLOAD_STEP_TIMEOUT_MS
    );
    assert_eq!(
        limits.report_attempt_timeout_for_payload(usize::MAX),
        DEFAULT_REPORT_ATTEMPT_TIMEOUT_MS
    );
    assert_eq!(
        limits.report_stage_timeout_for_attempt(REPORT_BASE_ATTEMPT_TIMEOUT_MS),
        REPORT_BASE_ATTEMPT_TIMEOUT_MS * u64::from(DEFAULT_REPORT_MAX_ATTEMPTS)
    ...
    fn new(
        ...
        let editorial = report.as_ref().and_then(|report| {
            report
                .as_ref()
                .ok()
                .map(|proposal| Ok(passing_editorial_plan(proposal)))
        });
        Self {
...
fn passing_editorial_plan(proposal: &Value) -> Value {
    let claims = proposal
        .get("claims")
...
fn workflow_args(query: &str) -> Value {
    ...
    serde_json::json!({
        ...
        "limits": {
            "timeoutMs": DEFAULT_PLANNED_RETRIEVAL_STAGE_TIMEOUT_MS,
            "maxToolCalls": 64,
