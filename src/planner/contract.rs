...
mod tests {
    ...
    fn planner_contract_reserves_official_source_queries_for_named_comparisons() {
        ...
        assert!(prompt.contains("capacity ceiling, not a breadth target"));
        assert!(prompt.contains("Prefer fewer complete analytical tracks"));
        assert!(
            prompt.contains("consolidate tracks instead of knowingly producing shallow sections")
        );
